...
from states import DifferemtialDriveState
...
def main():
    ...
    delivery_robot = SimpleVehicleSprite(str(project_root) + "/assets/skid_mobile_robot.png", 90, (100, 100))

    """ Declaring initial and final tates"""

    initial_state = DifferemtialDriveState(100, 100, radians(0), 0, 0, 0)

    # Multiple goals for testing
    # goal_state = DifferemtialDriveState(120, 300, radians(-90), 0, 0, 0)
    goal_state = DifferemtialDriveState(300, 600, radians(0), 0, 0, 0)
    # goal_state = DifferemtialDriveState(720, 220, radians(90), 0, 0, 0)
    ...
    while run:
        ...
        if path_ready == 0 and not stop:
            path_ready, path, explored_states = search_path(delivery_robot, obstacles, goal_state)
            ...
            if start_visualizing:
                for i in range(1, len(path)):
                    ...
                    while current_time < state.dt:
                        ...
                        dangle, dx, dy = prev_state.get_steps(current_time, state.ul, state.ur, prev_state.angle)
                        x = prev_state.x + dx
                        y = prev_state.y + dy
                        ...
                        pygame.event.pump()
                        render_all(screen, clock, delivery_robot, obstacles, explored_states, points_in_route)

                path_ready = 0
                stop = 1
        elif not stop:
            render_all(screen, clock, delivery_robot, obstacles, explored_states, [])
