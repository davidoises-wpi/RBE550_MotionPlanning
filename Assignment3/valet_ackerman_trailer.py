...
from vehicles import TrailerVehicleSprite
from obstacles import Obstacle
from states import AckermannTrailerDriveState
...
def render_all(window, clock, vehicle, obstacles, explored_states, points_in_route):
    ...
    if points_in_route:
        ...
        vehicle.set_orientation(degrees(point[2]), degrees(point[3]))
...
def main():
    ...
    obstacles = []
    obstacles.append(Obstacle((400+450,250), 400, 400, RED))
    obstacles.append(Obstacle((400+100,610), 200, 80, RED))
    obstacles.append(Obstacle((400+SCREEN_WIDTH/2,SCREEN_HEIGHT+1), SCREEN_WIDTH, 2, RED))
    obstacles.append(Obstacle((400+SCREEN_WIDTH/2,-1), SCREEN_WIDTH, 2, RED))

    car = TrailerVehicleSprite(str(project_root) + "/assets/tesla.png", str(project_root) + "/assets/tesla.png", 180, 180, (100, 100))

    """ Declaring initial and final tates"""

    initial_state = AckermannTrailerDriveState(100, 100, radians(0), radians(0), 0, 0, 0)

    # Multiple goals for testing
    # goal_state = AckermannTrailerDriveState(100+150, 400, radians(-90), radians(-90), 0, 0, 0)
    goal_state = AckermannTrailerDriveState(400+600, 600, radians(0), radians(0), 0, 0, 0)
    # goal_state = AckermannTrailerDriveState(400+720, 220, radians(90), radians(90), 0, 0, 0)
            ...
            if start_visualizing:
                for i in range(1, len(path)):
                    ...
                    while current_time < state.dt:
                        ...
                        dangle, trailer_dangle, dx, dy = prev_state.get_steps(current_time, state.psi, state.v, prev_state.angle, prev_state.trailer_angle)
                        x = prev_state.x + dx
                        y = prev_state.y + dy
                        angle = prev_state.angle + dangle
                        trailer_angle = prev_state.trailer_angle + trailer_dangle
                        points_in_route.append((x,y,angle, trailer_angle))
